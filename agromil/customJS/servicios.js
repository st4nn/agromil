function iniciarModulo()
{
    ...
    $(document).delegate('.btnProductos_Editar', 'click', function(event) 
    {
        ...
        $("#txtModal_ServicioPublicoAgregar_id").val($(this).attr("idProducto"));
    ...
    $(document).delegate('.btnProductos_Borrar', 'click', function(event) 
    {
        ...
        swal({
            ...
            $.post('server/php/proyecto/modals/borrarElemento.php', {Usuario: Usuario.id, idObj : $(fila[1]).text(), Tabla : 'serviciosPublicos'}, function(data, textStatus, xhr) 
            {
                if (isNaN(data))
                {
