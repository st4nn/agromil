<?php 
   ...
   if ($Produccion == 0)
   {
      $mensaje .= "<br>El día de hoy no se registró producción";
   } else
   {
      $mensaje .= "<br>El día de hoy se registró la siguiente producción:";
   ...
   if ($Despachos == 0)
   {
      $mensaje .= "<br><br>El día de hoy no se registraron despachos";
   } else
   {
      $mensaje .= "<br><br><br>El día de hoy se registraron los siguientes despachos:<br>";
      $tmpCliente = "";
   ...
   if ($materiaPrima == 0)
   {
      $mensaje .= "<br>El día de hoy no se registraron movimientos de Materia Prima";
   } else
   {
      $mensaje .= "<br><br>El día de hoy se registraron los siguientes movimientos de Materia Prima:<br><br>";
      ...
      foreach ($materiaPrima as $key => $value) 
      {
         ...
         $mensaje .= '<td style="border-top: solid 1px; border-left: solid 1px;">'. number_format($value->Ingresos, 2, ',', '.') . '</td>';
         $mensaje .= '<td style="border-top: solid 1px; border-left: solid 1px;">'. number_format(($value->Salidas *-1), 2, ',', '.') . '</td>';
         $mensaje .= '</tr>';
      }
   ...
   {
      $mensaje .= "<br>El día de hoy no se registró pago de Servicios Públicos";
   }
   else
   {
      $mensaje .= "<br><br>El día de hoy se registró el pago de los siguientes Servicios:<br><br>";
      $mensaje .= '<table></tbody>';
   ...
   else
   {
      ...
      foreach ($stockBodega as $key => $value) 
      {
         ...
         $mensaje .= '<td style="border-top: solid 1px;">'. $value->NombreReferencia . '</td>';
         $mensaje .= '<td style="border-top: solid 1px; border-right: solid 1px;">' . number_format($value->Sacos, 2, ',', '.') . ' Sacos</td>';
      }
   ...
   else
   {
      ...
      foreach ($stockMateriaPrima as $key => $value) 
      {
         ...
         $mensaje .= '<td style="border-top: solid 1px; border-right: solid 1px;">' . number_format($value->Cantidad, 2, ',', '.') . ' ' . $value->Unidades . '</td>';
      }
   ...
   echo "Consolidado del Mes " . date('m \d\e Y');
   ...
   $obj = EnviarCorreo($correo, "Consolidado del día " . date('d \d\e\l m \d\e Y'), $mensaje) ;
